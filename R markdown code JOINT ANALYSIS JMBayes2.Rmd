...
## ADJUSTED COX WITH TIME-VARYING PH REGRESSION ANALYSIS INCLUDING eGFR
```{r}
...
## ADJUSTED COX WITH TIME-VARYING PH REGRESSION ANALYSIS NOT INCLUDING eGFR
```{r }
...
summary(jointFit1)
```
## ADJUSTED JOINT LONGITUDINAL SURVIVAL ANALYSIS - normalized
area/cumulative-effect eGFR
```{r }
summary(jointFit2)
```
...
# CALCULATE P values and 95% cridible intervals
```{r }
...
## CRUDE COX WITH TIME-VARYING PH REGRESSION ANALYSIS
```{r }
CoxFit <- coxph(Surv(tstart, tstop, status) ~ LOWC3, 
	data = sle_05_time_varying, cluster = id)
```	

## CRUDE COX WITH TIME-VARYING PH REGRESSION ANALYSIS INCLUDING eGFR
```{r }
Stand.TV.CoxFit <- coxph(Surv(tstart, tstop, status) ~ LOWC3 + eGFR, 
	data = sle_05_time_varying, cluster = id)
```	
...
summary(CoxFit)
```	
## stabdard Cox PH model adjusted
```{r }
summary(CoxAFit)
```	
...
summary(jointCFit1)
```	
## CRUDE JOINT LONGITUDINAL SURVIVAL ANALYSIS - normalized
area/cumulative-effect eGFR
```{r}
summary(jointCFit2)
```	
